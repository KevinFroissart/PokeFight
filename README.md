...
Former
les membres du groupe à Git par un des membre qui l’utilise
régulièrement.
...
Nous
avons tous pu efficacement utiliser les fonctions de base de Git.

###
Quelle décision prenons nous suite à cette expérience ?

Il
faut que les gens spécialisés dans certains domaines puissent
...
Faire
des binômes pour augmenter l’efficacité générale et avoir un
code plus propre
...
Nous
n'avons pas fait de readMe pour le premier sprint alors celui-ci
...
Ensuite
nous avions décidé de chacun travailler de son côté sur chacun
...
Pour
le second sprint nous avons décidé de changer nos méthodes de
...
Ensuite
nous avons décidé de travailler par binôme. Ça nous a permis
...
le
code est plus propre et plus lisible par l’équipe , et que l’on
a pas besoin d’aller régulièrement régler des bug. 
...
Nous
allons plus régulièrement travailler en binômes pour les parties
les plus complexe.
...
Mettre
un réveil pour fusionner les classes proprement 10 minutes avant la
démo.
...
Pour
le Sprint 3 du mardi matin , nous avons repris les US qui n’ont pas
été finalisés pendant la journée et le sprint précédent, régler
les bugs qui étaient présent et commencé de nouveaux US.

On
a recommencé à travailler chacun de son côté tout en mettant en
...
Pour
les difficultés rencontrés, nous avons régler l’assemblage des
...
Nous
avons donc décider de mettre un réveil pour mettre en commun lors
du prochain sprint les codes 10 minutes avant la démo et la
rétrospective.

Nous
nous sommes trouvé performant sur notre entraide quand il y a des
...
Ce que nous allons faire durant le prochain sprint
	...
	-Avoir
un jeu équilibré et pouvoir finir le jeu

	-Voir
sa jauge de vie

	-Pouvoir
utiliser toutes les features sans bug
...
la
fusion est propre et qu’il n’y a pas eu de bug durant la
prochaine démo.
...
Rajouter
des US car il nous restait seulement 7 point d’action à réaliser

###
A quoi verra-t-on que cela à fonctionné ?

Si
nous avons du travail jusqu’au dernier Sprint

Nous
avons mis en place le réveil 10 minutes avant la fin du sprint afin
...
Nous
avons rencontré des difficultés avec Git pendant ce sprint à cause
...
Ce que nous avons fait durant ce sprint
	...
	-Avoir
un jeu équilibré et pouvoir finir le jeu en tant qu’utilisateur
...
Ce que nous allons faire durant le prochain sprint
	...
	-Avoir
un jeu fini sans bug du début à la fin
...
Il
nous restait des choses à ajouter et maintenant nous avons du
...
Ne
pas hésiter à mettre plus d’US dès le départ.

###
Qu’allons nous tester durant les 2 prochaines heures ?

Faire
la moitié du groupe pour le test des codes et l’autre qui règle
les problèmes.
...
Au
départ du sprint nous n’avions quasiment plus de US . 

Nous
avons donc choisit de rajouter des histoires utilisateurs pour
pouvoir compléter notre jeu , rajouter des utilitaires et le rendre
plus agréable à jouer.

Nous
avons aussi décider de régler le reste des bugs qui s’étaient
ajouter  au fur et à mesures des mises à jours du git et qui
étaient passé entre les mailles du filet.

Nous
avons donc un petit jeu presque viable mais pour le moment il manque
...
On
a réussi à compléter un nombre de point près de la moyenne des
sprint et qu’il y a moins d’erreur à l’application à la fin
du sprint.
...
Essayer
de faire ça pendant 10 minutes par sprint plutôt que de devoir tout
faire à la fin,

ce
qui permet de conservé une dépense de point tout au long.

###
Qu’allons nous tester durant les 2 prochaines heures ?

Se
répartir toutes les taches dès le début pour être le plus
...
Si
personne n’a rien à faire à un moment donné

Pour
ce dernier sprint de la deuxième journée nous avons décidé de
nous plonger dans le code pour régler tous les petits problèmes
visible et pour optimiser ce qui était possible.

Nous
avons rencontré un problème c’est que parfois certains ne
